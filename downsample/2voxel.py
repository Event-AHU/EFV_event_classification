...
if __name__ == '__main__':
    ...
    data_path = r"/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c0/DATA/dataset/Poker_Event/poker_event/"
    #save_path = r"/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c0/DATA/dataset/Poker_Event/poker_voxel/"
    save_path = r"/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c0/DATA/lidong/event_har/node1000/Poker_voxel1"
    ...
    for action_dir in action_dirs:
        ...
        #'/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c0/DATA/dataset/Poker_Event/poker_event/action_ex_001_Bombing_Variegata'
        video_files = os.listdir(action_path) #抽取前十个
       # video_files = os.listdir(action_path)
        ...
        for videoID in range(len(video_files)):
            ...
            if not os.path.exists(os.path.join(save_path, action_dir, foldName+'_voxel')):
                os.makedirs(os.path.join(save_path, action_dir, foldName+'_voxel'))
            else:
                continue
            mat_save = os.path.join(save_path, action_dir, foldName+'_voxel/')
           
            read_path = os.path.join(action_path, foldName)
            ...
            events = np.hstack([packet for packet in f['events'].numpy()]) #(527813,)
            breakpoint()
            ...
            for frame_no in range(0, int(frame_num / dvs_img_interval) - 1):
                ...
                if start_idx == end_idx:
                    ...
                    scipy.io.savemat(mat_save + '{:0>8d}.mat'.format(frame_no), mdict={'coor': np.zeros([100, 3]),
                ...
                #     continue
                data_dict = {'points': current_events}  #points:torch.Size([17294, 4])

                coor, features = transform_points_to_voxels(data_dict=data_dict, voxel_generator=voxel_generator,
                                                            device=device)
                
                #coor.shape:torch.Size([5000, 3]) feature.shape:torch.Size([5000, 16])
                # pdb.set_trace()
                coor = coor.cpu().numpy()
                features = features.cpu().numpy()
              
                # print('coor', coor)
                scipy.io.savemat(mat_save + '{:0>8d}.mat'.format(frame_no), mdict={'coor': coor, 'features': features})   # coor: Nx(t,x,y);   features:Nx32 or Nx10024
