    ...
    for classID in range(len(class_dir)):
        ...
        for FileID in tqdm(range(len(fileLIST))):
            ...
            if os.path.exists(os.path.join(save_path,'v2g_8_r3_base', cls_name, '{}.pt'.format(save_name))):
                continue
            # 
            file_subfolders = os.listdir(os.path.join(data_path, cls, file_Name))  # 使用file_Name作为子文件夹的名字
            for subfolder in file_subfolders:
                mat_file_name = '{}'.format(str(subfolder).zfill(8))  # 根据你提供的文件路径，mat文件名应该是8位数字
                if os.path.exists(os.path.join(data_path, cls, file_Name, mat_file_name)):
                    read_path = os.path.join(data_path, cls, file_Name, mat_file_name)
            ...
            #Data(edge_index=[2, 268], pos=[50, 3], x=[50, 16], y=[1]) 数据格式 
            G_save_path = os.path.join(save_path, 'v2g_8_r3_base', cls_name, '{}.pt'.format(save_name))
