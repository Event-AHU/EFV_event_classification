...
class CrossReplaceTransformerLayer(nn.Module):
    ...
    def _cr_block(self, x1, x2, attn_weight1, attn_weight2):
        ...
        cls_weight1 = attn_weight1[:, 0, :].transpose(0, 1)  # 交换维度
        cls_weight2 = attn_weight2[:, 0, :].transpose(0, 1)  # 交换维度
        #print('cls_weight1', cls_weight1.shape) #cls_weight1 torch.Size([2, 32])
        #print(cls_weight1)
        #print('cls_weight2', cls_weight2.shape) #cls_weight2 torch.Size([1, 9129])
        #print(cls_weight2)
        #print (x1.shape,x2.shape)
        x1_mean = torch.mean(x1, dim=-2)
        x2_mean = torch.mean(x2, dim=-2)
        # print('self.theta[0]',self.theta[0])
        # print('self.theta[1]',self.theta[1])
        for i in range(cls_weight1.shape[0]):
            if self.use_quantile:
                theta1 = np.quantile(
                    cls_weight1[i][1:].detach().cpu().numpy(), self.theta[0])
                theta2 = np.quantile(
                    cls_weight2[i][1:].detach().cpu().numpy(), self.theta[1])
            else:
            ...
            for j in range(1, cls_weight1.shape[1]):
                ...
                elif cls_weight1[i][j] < theta2:
                    x1[i][j] = self.mlp(x2_mean[i]).squeeze() + x1[i][j]

                if cls_weight2[i][j] < theta1:
                    x2[i][j] = x1_mean[i] + \
                        x2[i][j] if self.skip_connection else x1_mean[i]
                elif cls_weight2[i][j] < theta2:
                    x2[i][j] = self.mlp(x1_mean[i]).squeeze() + x2[i][j]
    ...
    def forward(self, src1, src2, replace=False, src1_mask=None, src1_key_padding_mask=None, src2_mask=None, src2_key_padding_mask=None):
        ...
        if self.norm_first:
            ...
            # print(attn_weight1.detach().cpu().numpy())
            # print(attn_weight1[0].shape)
            #print (x1.shape)#32,6,768
            if replace:
